...
export function Sidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  const pl     = isOpen ? PL_EXPANDED : PL_COLLAPSED
  ...
  return (
    <aside
      className={cn('sidebar-wrap', mobileOpen && 'mobile-open')}
      ...
      style={{
        ...
        gap: 4, flexShrink: 0, position: 'relative', zIndex: 20,
    ...
    >
      {/* Logo */}
      <Link href="/" style={{ textDecoration: 'none', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 6 }}>
      ...
      {NAV.map(item => {
        ...
        return (
          <div key={item.href} title={isOpen ? undefined : item.label}>
            <Link href={item.href} style={{ textDecoration: 'none' }} onClick={() => onClose?.()}>
              <div
                className={cn('nav-item', isActive && 'active')}
                style={{ width: '100%', justifyContent: 'flex-start', paddingLeft: pl, gap: 10, transition: 'padding-left 0.25s ease, background 0.2s ease, color 0.2s ease' }}
              >
      ...
      {/* Settings */}
      <div title={isOpen ? undefined : 'Settings'}>
        <Link href="/dashboard/settings" style={{ textDecoration: 'none' }} onClick={() => onClose?.()}>
          <div
            ...
            style={{ width: '100%', justifyContent: 'flex-start', paddingLeft: pl, gap: 10, transition: 'padding-left 0.25s ease' }}
          >
      ...
      {/* Avatar */}
      <div
        style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10, paddingLeft: pl, cursor: 'pointer', transition: 'padding-left 0.25s ease' }}
        title={isOpen ? undefined : 'Peter Karanja'}
      >
        <div style={{
        ...
        <div style={{ overflow: 'hidden', maxWidth: isOpen ? 140 : 0, opacity: isOpen ? 1 : 0, transition: 'max-width 0.25s ease, opacity 0.15s ease' }}>
